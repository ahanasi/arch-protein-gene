...
from parameterize_distribution_by_MLE import *
from scipy import stats
...
def poisson_mle_lrt(file_string):
    ...
    m = np.log2(np.divide((mle_stats[:,1]+1),(mle_stats[:,0]+1)))
    a =  0.5*(np.log2(mle_stats[:,1]+1) + np.log2(mle_stats[:,0]+1))

    pyplot.figure()
    pyplot.scatter(a,m)
    pyplot.scatter(a[np.where(pvalues_poisson_mle_lrt[:,0] >= -10*np.log2(0.05))],m[np.where(pvalues_poisson_mle_lrt[:,0] >= -10*np.log2(0.05))],color='red')
    pyplot.title('MA Plot: ES vs. NPC - pvalues from Poisson log-likelihood ratio test code')
    pyplot.show()
    ...
    return pvalues_poisson_mle_lrt
